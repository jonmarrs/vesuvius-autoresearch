...
def probe_data():
    ...
    for i in range(5):
        x, target = ds[i]
        print(f"Sample {i}:")
        print(f"  x shape: {x.shape}, max: {x.max():.4f}, min: {x.min():.4f}, mean: {x.mean():.4f}")
        if target is not None:
            print(f"  target shape: {target.shape}, max: {target.max():.4f}, min: {target.min():.4f}, sum: {target.sum():.4f}")
