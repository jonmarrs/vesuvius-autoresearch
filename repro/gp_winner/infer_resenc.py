...
class CFG:
    ...
    comp_name = "vesuvius"
    comp_dir_path = "./"
    comp_folder_name = "villa/ink-detection/"
    comp_dataset_path = "villa/ink-detection/"
...
def read_image_mask(fragment_id, start_idx=17, end_idx=43, CFG=CFG):
    ...
    inklabel_files = glob.glob(
        CFG.comp_dataset_path + f"train_scrolls/{fragment_id}/*inklabels.*"
    )
    ...
    fragment_mask = cv2.imread(
        CFG.comp_dataset_path + f"train_scrolls/{fragment_id}/{fragment_id_}_mask.png",
        0,
    )
...
def worker_function(fragment_id, CFG):
    ...
    if not os.path.exists(CFG.comp_dataset_path + f"train_scrolls/{fragment_id}"):
...
class RegressionPLModel(pl.LightningModule):
    def __init__(self, pred_shape, size=256, with_norm=False):
        ...
        self.loss_func = lambda x, y: (
            0.5 * self.loss_func1(x, y) + 0.5 * self.loss_func2(x, y)
        )

        n_stages = 3
        features_per_stage = [64 * (2**i) for i in range(n_stages)]
    ...
    def forward(self, x):
        ...
        # Pad Z dimension from 26 to 28 so it is divisible by 4 (stride 2x2)
        x = F.pad(x, (0, 0, 0, 0, 1, 1))

        x = self.backbone(x)

        # Crop Z dimension back to 26
        x = x[:, :, 1:-1]

        # x is (B, 1, D, H, W)
        x = torch.mean(x, dim=2)  # (B, 1, H, W)
        return x
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, (x1, y1, x2, y2) in enumerate(xyxys):
            self.mask_pred[y1:y2, x1:x2] += y_preds[i].squeeze(0).numpy()
    ...
    def on_validation_epoch_end(self):
        ...
        print(
            f"Saving prediction to {CFG.model_dir}/resenc_unet_PHercParis2Fr143_pred.png"
        )
        cv2.imwrite(
            f"{CFG.model_dir}/resenc_unet_PHercParis2Fr143_pred.png",
            (self.mask_pred * 255).astype(np.uint8),
        )
...
for fid in fragments:
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ModelCheckpoint(
                filename=f"resenc_unet_{fid}_fr" + "{epoch}",
    ...
    trainer.validate(
        ...
        ckpt_path=f"{CFG.model_dir}/resenc_unet_PHercParis2Fr143_frepoch=3.ckpt",
    )

    # CSVLogger: nothing to finish
