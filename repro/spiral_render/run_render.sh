...
# this after `cd "$W/spiral-fitting"`, so $(dirname "$0") resolved relative to the
# work dir and PYTHONPATH pointed at a directory that does not exist. The flatten
# ran at stock speed under a flatten_deterministic=1 label -- caught only because
# the process environment was inspected directly instead of trusting the banner.
...
if [ "${FLATTEN_DETERMINISTIC:-}" = "1" ]; then
  echo "[run_render] FLATTEN_DETERMINISTIC=1: flatten will be bit-reproducible (~9.5x slower); shim=$SHIM_DIR" >&2
fi
PYTHONPATH="$SHIM_DIR:$W/vesuvius/src" exec "$VENV" -u render_ink.py "$W/meshes" \
