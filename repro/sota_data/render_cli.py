...
def infer_scale(seg, obj, volume, out_root, y0, x0, size, level, sign):
    ...
    for div in CANDIDATE_DIVS:
        ...
        try:
            out_seg, _ = render_region(seg, obj, volume, y0, x0, min(size, 1024), level,
                                       sign, out_root, frag_id=fid, obj_level_div=div)
    ...
    if best[0] is None:
        raise SystemExit("all candidate scales failed — check --obj / --volume / --level")
...
def main(argv=None):
    ...
    ap.add_argument("--obj", required=True, help="original.obj (local path or S3 key/url)")
    ...
    ap.add_argument("--frag-id", default=None, help="fragment id (default: obj basename)")
    ap.add_argument("--region", nargs=3, type=int, metavar=("Y0", "X0", "SIZE"),
                    default=[0, 0, 2048], help="render region + grid size (default 0 0 2048)")
    ap.add_argument("--level", type=int, default=2, help="volume pyramid level (default 2)")
    ...
    seg = (args.frag_id or os.path.basename(obj).replace("_original.obj", "")
           .replace(".obj", ""))
    y0, x0, size = args.region

    if args.scale == "auto":
        print("Inferring obj coordinate scale (teacher-free surface-structure probe)...",
              flush=True)
        div = infer_scale(seg, obj, args.volume, args.out, y0, x0, size, args.level, args.sign)
    else:
        div = float(args.scale)

    out_seg, stats = render_region(seg, obj, args.volume, y0, x0, size, args.level,
                                   args.sign, args.out, frag_id=seg,
                                   obj_level_div=div, extra_prov={"cli": True})
    print(f"\nRendered {out_seg}")
    print(f"  layers: 26  valid_frac={stats['valid_frac']:.3f}  "
          f"clamped_frac={stats['clamped_frac']:.3f}  obj_level_div={div}")
    print("  label-free (no ink GT fabricated). Renderer is placement-validated on Scroll 1 "
