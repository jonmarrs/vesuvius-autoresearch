...
TARGETS = {
    "orig": {
        ...
        "extra_disclosure": (
            ...
            "optimistic.\n\n"
            "**PLACEMENT: 46.6 level-2 px — passes the 48 px gate by only 1.4 px.** Read "
            "this row's absolute values with more caution than the held-out target's, which "
            "sits at 32.0 px. This segment was *not* affected by the 2026-08-07 "
            "`LEVEL0_SHAPE` bug (the hardcoded constant was its own geometry), so its offset "
            "is a separate matter: the cross-scan floor simply varies more by segment than "
            "the held-out measurement suggested. **The threshold was deliberately "
            "NOT raised to give this row headroom.**\n\n"
            "**The global figure understates the local error here.** Per-768px-tile "
            "placement (`scripts/probe_placement_field.py`) has sd 26.8 (dy) / 33.0 (dx) "
            "with individual tiles reaching ~100 px — **~0.96 mm** — against a global 46.6 px "
            "/ 0.45 mm. A fitted plane leaves scatter equal to the raw scatter, so this is "
            "non-rigid: not a constant offset and not a scale error, and there is no "
            "convention bug left to find. The held-out target's field is 3-4x tighter "
            "(sd 8.2 / 9.5). **Treat scores on this target as indicative only.** "
            "See `MAX_PLACEMENT_OFFSET_L2PX` and "
    ...
    "heldout": {
        ...
        "extra_disclosure": (
            ...
            "the claim.\n\n"
            "**⚠ These numbers REPLACE the 2026-07-07 version of this report, which was "
            "invalid.** That run used a hardcoded `LEVEL0_SHAPE` belonging to segment "
            "20230702185753, so this segment's region crop was scaled wrongly and the "
            "registered label came out displaced and stretched (x +9.8%, y +0.79%). It "
            "produced the since-retracted claim that everything reads at chance here. Fixed "
            "2026-08-07 (`LEVEL0_SHAPES` is now per-segment and `_set_target` refuses an "
            "unrecorded segment); see `reports/detector/registration_offset_2026-08-07.md`.\n\n"
            "**The enrichment gate was RIGHT and we overrode it.** The 2026-07-07 run read "
            "teacher-enrichment 1.68, failed the enrichment gate, and we explained that away "
            "as 'the canon teacher reads this segment poorly' -- then built a teacher-free "
            "gate to get past it. The gate was not false-negativing: it was correctly "
            "detecting the broken registration. On the fixed pipeline the SAME convention "
            "scores **enrichment 6.01** (vs 1.77/1.84/1.61 for the alternatives -- now "
            "decisive, where before it was marginal), comfortably clearing the gate it "
            "previously failed. The teacher-free gate is retained because "
            "teacher-independence is genuinely better methodology, not because enrichment "
            "cannot be used here.\n\n"
            "Registration is validated on the codified teacher-free gate "
            "(`gate_mode=teacher_free`): 3D correspondence residual 7.95 old-scan voxels and "
            "registered-label text-line periodicity 0.867. **Scope of this evidence:** "
        ...
        "binary_caveat": (
            ...
            "all-positive predictor scores F1 ≈ 0.31 -- the legacy detector predicts "
            "all-positive and sits exactly there, which is the floor to compare against. "
            "The robust reads are AP-prevalence-lift and ROC-AUC. The canon teacher is a "
            "BINARY map, so ROC-AUC and AP structurally understate it relative to the "
            "students' continuous probability maps; the fair teacher-vs-student comparison "
            "is `f1_at_0.5`."
        ),
    },
}
...
def cmd_score():
    ...
    lines = [
        ...
        "approximate. NOTE: the residual measures correspondence SCATTER, not absolute "
        "placement -- a label can have a tight residual and still be bodily displaced, "
        "which is exactly the 2026-08-07 LEVEL0_SHAPE bug. Scatter depresses rows about "
        "equally; a systematic offset does NOT, and can reorder them. Check placement "
        "with `scripts/probe_registration_offset.py`, never with the residual alone). "
        "The 'canon teacher' row scores the released model prediction itself against "
        "human labels.",
        "",
        EXTRA_DISCLOSURE,
        "",
