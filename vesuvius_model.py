...
class InkDetectorOptimized(nn.Module):
    def __init__(self, config, base_feat=32, num_blocks=8, num_heads=8, dropout=0.0):
        ...
        # Progressive UNet Decoder with Dual-Scale Skip Connections
        # Stage 1: 1/4 -> 1/2
        self.up1 = nn.ConvTranspose3d(base_feat, base_feat // 2, kernel_size=4, stride=2, padding=1)
        self.fusion1 = nn.Conv3d(base_feat, base_feat // 2, kernel_size=1) # Concat s1 + up1
        
        # Stage 2: 1/2 -> 1/1
        self.up2 = nn.ConvTranspose3d(base_feat // 2, base_feat // 4, kernel_size=4, stride=2, padding=1)
        # We also skip connect from raw input (1 channel)
        self.fusion2 = nn.Conv3d(base_feat // 4 + 1, base_feat // 4, kernel_size=1)
    ...
    def forward(self, x, return_fiber=False, return_qc=False, **kwargs):
        ...
        s1 = self.stage1(x)    # [B, d/2, Z/2, H/2, W/2]
        x_emb = self.stage2(s1) # [B, d, Z/4, H/4, W/4]
        ...
        # 3. Progressive UNet Decoding
        x_trans = x_flat.transpose(1, 2).reshape(B, -1, lz, lh, lw)
        
        # Upsample 1/4 -> 1/2 and fuse with s1
        x_up1 = self.up1(x_trans)
        x_f1 = self.fusion1(torch.cat([x_up1, s1], dim=1))
        
        # Upsample 1/2 -> 1/1 and fuse with raw input x
        x_up2 = self.up2(x_f1)
        x_f2 = self.fusion2(torch.cat([x_up2, x], dim=1))
...
class ResBlock3D(nn.Module):
    def __init__(self, channels):
        super().__init__()
        # GroupNorm is better than InstanceNorm for small batches
...
def mission_critical_audit(bench_only=False):
    ...
    print(f"Architecture: Full-Progressive UNet-Transformer (Params: {sum(p.numel() for p in model.parameters())/1e6:.2f}M)")
    if not bench_only:
