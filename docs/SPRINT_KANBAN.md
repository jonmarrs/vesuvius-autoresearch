...
- **[Sprint 034] Volume Cartographer Native Bridge & Public Docs Cleanup** *(PR #916 follow-up / upstream alignment)*: (TODO) Treat closed PR #916 as obsolete and keep new data-access work on the official `volume-cartographer` / VC3D path. Acceptance gate: (a) profile current `volume_cartographer_wrapper` local OME-Zarr reads during prize workloads, (b) build a native Volume Cartographer bridge only if Python/Zarr I/O is a prize-throughput bottleneck or if it materially improves upstreamability, (c) update/supersede public-facing prize docs that still present `vesuvius-c` as the current path, (d) prepare a short PR #916 follow-up comment only after there is a concrete Volume Cartographer-aligned replacement, benchmark, or documentation artifact to link.
