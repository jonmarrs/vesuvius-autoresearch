...
- **[Sprint 036] Human-Evaluated Primus Inference Repair for Villa PR #899** *(review trust / optimized inference)*: (ACTIVE) Repair open PR #899 before replying to the reviewer. Repair slices pushed to Jon's Villa fork: `f1c1aa371 test optimized inference Primus loader contract` and `02b2c3ce2 validate Primus loader against real Villa network`. Current evidence adds `PR899_HUMAN_EVALUATION.md`, fixes the Primus wrapper's tensor-shape handling, adds focused loader/shape tests, hardens the ConfigManager shim required by Villa's real `NetworkFromConfig`, and adds an optional integration test that builds/reloads a minimal real Primus-S checkpoint envelope. Remaining acceptance gate: (a) remove/rewrite generated-content footers from the PR narrative, (b) either install/vendor the required `vesuvius` dependency in the optimized inference image or narrow the PR so it does not claim runnable container support, (c) run an end-to-end optimized inference Docker smoke test when Docker is available, (d) only then post a short reviewer reply acknowledging the issue and linking the new evidence.
