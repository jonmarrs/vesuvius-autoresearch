...
- **[Sprint 037] Fresh Primus Optimized-Inference Replacement PR** *(PR #899 successor / trust recovery)*: (TODO) Only open a new PR after there is a clean branch with no generated-content footers, a rewritten human PR body, and a passing end-to-end Docker smoke on a host with working container execution. Acceptance gate: (a) start from current `ScrollPrize/villa:main`, not the stale closed PR branch, (b) cherry-pick or reimplement only the reviewed minimal Primus loader/dependency/test pieces from `jonmarrs/villa:primus-loader-optimized-inference`, (c) run the full 14-test Primus/runtime/smoke diagnostic suite, (d) run `smoke_primus_docker.sh` to completion with the Primus-enabled GPU image, (e) include `PR899_HUMAN_EVALUATION.md`-style reasoning without AI footers or defensive language, (f) open a fresh replacement PR and only then comment on closed PR #899 with a short link to the replacement.
