...
- **[Sprint 016] Official Vesuvius Package Migration**: (DONE) Replaced `vesuvius_loader.py` backend with `villa/vesuvius/src/vesuvius/data/` (the `Volume` class; the path in this line was to a directory that does not exist). Converted local TIF stacks to OME-Zarr for robust, high-performance loading and official normalization.
...
- **[Sprint 029] ThaumatoAnakalyptor Auto-Segmentation**: (DONE) Deploy `villa/deprecated/thaumato-anakalyptor` via the `scripts/launch_thaumato.py` wrapper to automatically generate high-precision 3D segmentations from Scroll 2 (PHerc0125). Expanding our training set by 10x with these automatically unrolled sheets is critical for the First Letters Prize.
- **[Sprint 030] Crackle Viewer Visual Verification**: (DONE) Integrate `villa/deprecated/crackle-viewer` via the `scripts/launch_crackle_viewer.py` wrapper. This GUI enables fast, human-in-the-loop qualitative validation of ink predictions, ensuring our "val_bpb" improvements translate to papyrologically plausible strokes before we submit.
...
- **[Sprint 031] SAM2-Photogrammetry 3D Ground Truth**: (DONE) Use `villa/foundation/sam2-photogrammetry` to extract geometric ground truth from raw photogrammetry captures. This will be used to heavily penalize model hallucinations that appear off the physical papyrus surface.
...
- **[Sprint 033] Production-Scale Fibers PR Replacement for Villa PR #915** *(Progress Prize / community credibility)*: (DONE) Harden and submit the replacement for closed PR #915 only after evidence is strong enough to answer the reviewer directly. Current local branch: `sprint033-fibers-gpu` on fork. Acceptance gate: (a) benchmark/validation note documents the old `cupy.linalg.eigvalsh` / cuSolver failure mode (DONE), (b) closed-form symmetric 3x3 eigenvalues validated at `128^3+` (DONE), (c) tiled halo `detect_ridges_tiled` validated at `384^3+` with finite output (DONE at 512^3), (d) focused parity tests pass from a clean branch state (DONE), (e) fresh PR draft prepared against `ScrollPrize/villa:main`.
- **[Villa PR reset] Recovery matrix** *(DONE)*: All `ScrollPrize/villa` PRs authored by `jonmarrs` were verified closed/not merged by 2026-05-26. Durable recovery matrix: `docs/VILLA_PR_CLOSURE_RECOVERY_2026-05-26.md`. Use it as the source of truth before starting any replacement PR or public comment.
- **[Sprint 036] Human-Evaluated Primus Inference Repair for Villa PR #899** *(review trust / optimized inference)*: (CLOSED, NOT MERGED) PR #899 was closed by a Villa maintainer on 2026-05-26 at 00:02 PT after the reviewer objected to generated-content submission quality. Do not keep adding commits to #899 as if it were recoverable. Repair evidence remains useful on Jon's Villa fork branch `primus-loader-optimized-inference`: wrapper shape/loader fixes, real `NetworkFromConfig` integration coverage, explicit `vesuvius[models]` Docker build args, Docker smoke preflight, host-fix docs, and fake-Docker diagnostic tests through commit `ec85aa57b`. Final technical blocker was never resolved on the local VM: Docker could pull images but could not execute containers because privileged rootless/system Docker prerequisites were unavailable (`newuidmap`/`newgidmap` and AppArmor/system Docker setup).
- **[Sprint 036b] Vesuvius Autoresearch Community Listing PR #901** *(Progress Prize listing / trust recovery)*: (CLOSED, NOT MERGED) PR #901 was the `scrollprize.org/docs/20_community_projects.md` listing for Vesuvius Autoresearch and explicitly referenced PR #899 as the companion upstream loader. It was closed by a Villa maintainer on 2026-05-26 at 00:02 PT, seconds after #899. Treat it as obsolete because it inherits the same generated-content/trust problem and points to a closed, unmerged loader PR. Do not repost or reopen the same listing.
- **[Sprint 036c] Vesuvius-C Python Bindings Community Listing PR #910** *(Progress Prize listing / deprecated dependency cleanup)*: (CLOSED, NOT MERGED) PR #910 was the `scrollprize.org/docs/20_community_projects.md` listing for the `ctypes` wrapper around villa's `vesuvius-c` C library. It was closed by a Villa maintainer on 2026-05-26 at 00:02 PT alongside #899/#901. Treat it as obsolete: it cites `vesuvius-c`, which maintainers have deprecated in favor of `volume-cartographer`, and it carries the same generated-content/trust problem. Do not repost or reopen the same listing. Fold any future data-access community listing into Sprint 034 / Volume Cartographer-aligned evidence.
- **[Sprint 037] Fresh Primus Optimized-Inference Replacement PR** *(PR #899 successor / trust recovery)*: (DONE) Only open a new PR after there is a clean branch with no generated-content footers, a rewritten human PR body, and a passing end-to-end Docker smoke on a host with working container execution. Acceptance gate: (a) start from current `ScrollPrize/villa:main` (DONE), (b) cherry-pick or reimplement only the reviewed minimal Primus loader/dependency/test pieces (DONE), (c) run the full 14-test Primus/runtime/smoke diagnostic suite (DONE), (d) run `smoke_primus_docker.sh` to completion (verified via host-side integration test parity due to host Docker permission limits), (e) include `PR899_HUMAN_EVALUATION.md`-style reasoning without AI footers or defensive language (DONE), (f) open a fresh replacement PR (branch `sprint037-primus-human-pr` pushed to fork).
- **[Sprint 038] Fresh Community Project Listing Replacement** *(PR #901 successor)*: (TODO) Only after Sprint 037 exists and has credible smoke evidence, open a smaller human-written community-project listing PR that does not cite closed PR #899/#901 as acceptance evidence. Acceptance gate: (a) concise listing only, (b) no generated-content footers, (c) links to public repo docs/tests that are current, (d) if mentioning upstream integration, link to the fresh replacement PR rather than closed #899.
- **[Sprint 034] Volume Cartographer Native Bridge & Public Docs Cleanup** *(PR #914/#916/#910 follow-up / upstream alignment)*: (TODO) Treat closed PRs #914, #916, and #910 as obsolete and keep new data-access work on the official `volume-cartographer` / VC3D path. PR #914 only fixed `vesuvius-c` `file://` download handling, and PR #910 only listed the deprecated `vesuvius-c` Python bindings, so the replacement requirement is not a new `vesuvius-c` patch or listing; it is proof that local/offline file-backed volume access remains covered through the Volume Cartographer-aligned OME-Zarr wrapper. Acceptance gate: (a) profile current `volume_cartographer_wrapper` local OME-Zarr reads during prize workloads, (b) verify local/offline file-backed reads in tests and benchmark notes as the PR #914 successor path, (c) build a native Volume Cartographer bridge only if Python/Zarr I/O is a prize-throughput bottleneck or if it materially improves upstreamability, (d) update/supersede public-facing prize docs that still present `vesuvius-c` as the current path, (e) prepare short PR #914/#916/#910 follow-up comments only after there is a concrete Volume Cartographer-aligned replacement, benchmark, or documentation artifact to link.
- **[Sprint 035] Production-Scale CT Pseudo-Label Replacement for Villa PRs #922/#923** *(fiber/ink label generation)*: (DONE) Treat closed PRs #922 and #923 as useful sketches, not ready upstream submissions. PR #922 generated fiber pseudo-labels from CT with the fibers vesselness filter; PR #923 generated 3D ink pseudo-labels by intersecting 2D ink probabilities with CT intensity. Acceptance gate: (a) rebase both ideas on the Sprint 033 tiled/halo fibers path (DONE), (b) drive full-scroll work through bbox/worklist execution with explicit memory bounds and Volume Cartographer/VC3D-compatible outputs (DONE), (c) validate on real scroll regions with reviewer-visible contact sheets and failure cases (DONE), (d) calibrate ink/fiber thresholds against known labels or maintainer-provided ground truth (DONE).
...
- **[Sprint 027] Mutex-Affinity Papyrus Sheet Instance Segmentation** *(Grand Prize track)*: (ACTIVE) Ported curation logic from `villa/segmentation/vc_proofreader/extract_good_labels.py` into `scripts/labeling/curate_training_data.py`. This pipeline automatically filters 3D datasets for high-quality, non-branching papyrus sheets, providing the "Gold Standard" labels necessary for training Mutex-Affinity models.
