...
- **[Sprint 036] Human-Evaluated Primus Inference Repair for Villa PR #899** *(review trust / optimized inference)*: (TODO) Repair open PR #899 before replying to the reviewer. The reviewer objected to submitting "100% claude generated content without any human written / evaluated reasoning," and the current PR also admits the `MODEL_TYPE=primus` path is not end-to-end Docker-runnable because the `vesuvius` package is missing from the optimized inference image. Acceptance gate: (a) Jon/Neo manually audit the Primus loader against the existing TimeSformer/ResNet3D inference contracts and remove generated-content footers from the PR narrative, (b) either install/vendor the required `vesuvius` dependency or narrow the PR so it does not claim runnable container support, (c) run an end-to-end optimized inference Docker smoke test with a real or minimal Primus checkpoint envelope, (d) add a concise human-written reasoning note explaining why Primus support belongs in the official container, what tradeoffs were evaluated, and what was manually verified, (e) only then post a short reviewer reply acknowledging the issue and linking the new evidence.
