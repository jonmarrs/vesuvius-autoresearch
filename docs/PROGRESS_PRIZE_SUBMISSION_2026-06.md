...
**Status:** QUEUED for June filing on **four** villa PRs:
...
- [ScrollPrize/villa#922](https://github.com/ScrollPrize/villa/pull/922) — `generate_fiber_labels_from_ct.py`, addresses villa issue [#193](https://github.com/ScrollPrize/villa/issues/193) (`help wanted` / Progress Prize candidate).
- [ScrollPrize/villa#923](https://github.com/ScrollPrize/villa/pull/923) — `generate_3d_ink_labels.py`, addresses villa issue [#192](https://github.com/ScrollPrize/villa/issues/192) (`good first issue` + `help wanted`). Sketch PR explicitly seeking maintainer feedback on threshold calibration.
...
### 5. Sketch PR — `feat(fibers-dataset): generate 3D ink pseudo-labels from CT (no annotation required)`

Pull request: **https://github.com/ScrollPrize/villa/pull/923**
Addresses: **villa issue [#192](https://github.com/ScrollPrize/villa/issues/192)** — `good first issue` + `help wanted`.

#192 calls out that current ink labels are "generated by a human annotator drawing the letter they think the ink model most closely resembles" and asks for *as-accurate-as-possible ink labels that are representative of only the detectable ink patterns on the surface, and in true 3d rather than a single image projected across multiple layers.*

PR #923 adds `foundation/datasets/fibers-dataset/generate_3d_ink_labels.py`, a standalone CLI that combines the model's 2D ink probability with 3D CT intensity so labels only land where the model AND the CT agree there's ink material. Three optional refinements (surface-window thin-shell restriction, morphological close, 26-connected-component filter), each ablation-tested on a real Scroll 2 candidate. The surface-window refinement is dominant — it cuts 65% of baseline labels by rejecting bulk-papyrus noise. Four passing synthetic-CT smoke tests.

**Status: sketch.** Threshold choices are uncalibrated; the PR description names specific questions for `@hendrikschilling` / `@bruniss` about acceptable false-positive rates and whether per-column CT percentile is the right intensity model. Designed to start the #192 conversation, not close it. The script also runs locally as `scripts/generate_3d_ink_labels.py` in autoresearch, with ablation PNGs persisted at `reports/3d_ink_labels_ablation/`.

Ablation snapshot from PR description (Scroll 2 div_90 candidate, 64³ patch):

| config | labels | label_fraction |
|---|---|---|
| baseline | 24,990 | 9.53% |
| `--surface-window 4` | 8,718 | 3.33% |
| tight (surface=4, cc=16, close=1) | 8,162 | 3.11% — physically plausible ink density |
