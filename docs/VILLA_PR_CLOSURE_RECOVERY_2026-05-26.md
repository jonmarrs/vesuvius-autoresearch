...
2. **Rebuild #899 as a fresh current-main PR.** Carry over only the minimal Primus loader/dependency/test pieces, remove generated-content footers, write the PR body in human review language, and include the passing Docker smoke evidence. Prep branch exists on Jon's fork: `sprint037-primus-recovery`, rebased from current `ScrollPrize/villa:main` at `c0c6fafe3` and pushed after the 14-test Primus/runtime/smoke suite passed. Do not open a PR from it until Docker container execution works and the full Primus Docker smoke passes.
