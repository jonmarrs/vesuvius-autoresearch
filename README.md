...
- **Surface-volume renderer** (`repro/sota_data/render_cli.py`): turns the bucket's **mesh-only** segments (`original.obj` + volume, no surface volume) into detector-ready surface volumes — one command, teacher-free scale inference, label-free output; validated on Scroll 1 (NCC ~0.59). Makes previously-unusable segments (e.g. Scroll 3 / PHerc0332) readable. See **[docs/SURFACE_RENDERER.md](docs/SURFACE_RENDERER.md)**.
