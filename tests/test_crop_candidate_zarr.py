...
def test_crop_candidate_zarr_writes_bounded_crop(tmp_path):
    ...
    src = zarr.open(str(src_path), mode="w", shape=(10, 20, 30), chunks=(5, 10, 10), dtype="u1")
    data = np.arange(10 * 20 * 30, dtype=np.uint32).reshape(10, 20, 30) % 255
    src[:] = data.astype("u1")

    shape = crop_candidate_zarr(src_path, dst_path, z=2, y=6, x=8, depth=4, height=5, width=6)
