...
import analyse_same_winding_current as mod  # noqa: E402
...
def test_the_registered_arms_are_constants():
    assert mod.ABLATED_ARMS == ("nosamecur_s1", "nosamecur_s2", "nosamecur_s3")
    assert mod.BASELINE_ARMS == ("curbase_s1", "curbase_s2", "curbase_s3")
    assert len(mod.BASELINE_ARMS) == 3
    assert mod.REQUIRED_ABLATED == 3 and mod.REQUIRED_BASELINE == 3
...
def test_the_mde_reflects_three_versus_six():
    """3v3 here, not 3v6: the current-code control has three arms, so the MDE is
    back to ~9.6%."""
...
def test_a_partial_sample_is_refused(tmp_path):
    ...
    ink.update({t: 1.7e6 for t in mod.BASELINE_ARMS[:2]})
