...
VC_PROOFREADER_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '../villa/segmentation/vc_proofreader'))
...
def main():
    parser = argparse.ArgumentParser(description="Curate 3D papyrus sheet segmentation labels.")
    parser.add_argument('--input', type=str, required=True, help="Input 3D OME-Zarr containing segmentation labels (e.g., from Thaumato)")
    parser.add_argument('--output', type=str, required=True, help="Output path for the curated 'Gold Standard' Zarr")
    parser.add_argument('--chunk-size', type=int, default=64, help="Chunk size for curation evaluation")
    parser.add_argument('--min-percent', type=float, default=1.0, help="Minimum percentage of voxels labeled to keep chunk")
    parser.add_argument('--max-percent', type=float, default=95.0, help="Maximum percentage (avoids completely solid false-positive chunks)")
    parser.add_argument('--min-cc', type=int, default=1, help="Minimum number of connected components")
    parser.add_argument('--max-cc', type=int, default=5, help="Maximum connected components (rejects highly fragmented/crushed regions)")
    parser.add_argument('--reject-branches', action='store_true', default=True, help="Reject chunks with 2D skeletal branches (folds/merges)")
    parser.add_argument('--workers', type=int, default=4, help="Number of worker processes")
    
    args = parser.parse_args()

    print(f"--- Vesuvius Autoresearch: Automated Label Curation ---")
