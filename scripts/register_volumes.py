...
def main():
    ...
    parser.add_argument("--fixed", type=str, required=True, help="Path to the fixed (reference) Zarr volume")
    parser.add_argument("--moving", type=str, required=True, help="Path to the moving (source) Zarr volume to be aligned")
    parser.add_argument("--fixed-voxel-size", type=float, default=7.91, help="Voxel size of the fixed volume in microns")
    parser.add_argument("--output-transform", type=str, required=True, help="Output JSON path for the aligned transform matrix")
    ...
    print(f"--- Vesuvius Autoresearch Volume Registration ---")
    ...
    cmd = [
        "python3", "-i", os.path.join(villa_path, "find_transform.py"),
        "--fixed", args.fixed,
        "--fixed-voxel-size", str(args.fixed_voxel_size),
        "--moving", args.moving,
        "--output-transform", args.output_transform
    ]
    ...
    print("Perform coarse alignment using Alt+WASDQE, add landmarks with Alt+1/2, and the transform will be fit automatically.")
    ...
    try:
        ...
        print("You can now apply this transform inside vesuvius_loader.py using SimpleITK.")
