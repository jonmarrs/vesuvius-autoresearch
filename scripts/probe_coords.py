...
def probe_coords():
    ...
    print(
        f"Direct Zarr patch at first valid coord: max={patch.max()}, mean={patch.mean():.4f}"
    )

    # Check mask at this coord
    mask_patch = ds.mask[y0 : y0 + 64, x0 : x0 + 64]
    print(
        f"Mask patch at first valid coord: max={mask_patch.max()}, mean={mask_patch.mean():.4f}"
    )

    # Check ink at this coord
    ink_patch = ds.labels[y0 : y0 + 64, x0 : x0 + 64]
    print(
        f"Ink patch at first valid coord: max={ink_patch.max()}, mean={ink_patch.mean():.4f}"
    )
