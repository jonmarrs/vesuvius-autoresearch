...
for obj in resp.get('CommonPrefixes', []):
    ...
    if prefix.startswith('PHerc') and not 'Cr' in prefix and not 'Fr' in prefix and not 'Paris' in prefix:
...
def download_chunks_for_scroll(scroll_id, target_gb=1.0):
    ...
    if not valid_volume_prefix:
        print(f"  [{scroll_id}] No masked.zarr volume found in root. Trying samples/...")
    ...
    for page in pages:
        for obj in page.get('Contents', []):
            ...
            if downloaded_bytes >= target_bytes:
                print(f"  [{scroll_id}] Success: Downloaded {downloaded_bytes/1024/1024/1024:.2f} GB ({count} chunks).")
                return True
                
    if downloaded_bytes > 0:
        print(f"  [{scroll_id}] Success (Partial): Downloaded {downloaded_bytes/1024/1024/1024:.2f} GB ({count} chunks).")
        return True
    else:
