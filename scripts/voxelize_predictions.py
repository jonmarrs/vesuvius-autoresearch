...
def main():
    parser = argparse.ArgumentParser(description="Voxelize Ink Predictions and Prune Noise")
    parser.add_argument("--input", type=str, required=True, help="Path to input ink prediction Zarr volume")
    parser.add_argument("--output_obj", type=str, required=True, help="Output path for the generated 3D OBJ mesh")
    parser.add_argument("--threshold", type=float, default=0.5, help="Ink threshold for voxelization")
    ...
    prune_script = os.path.join(vesuvius_src_path, "vesuvius", "image_proc", "run", "filter_branching_labels.py")
    
    # 2. Voxelize
    voxelize_script = os.path.join(vesuvius_src_path, "vesuvius", "image_proc", "run", "voxelize_objs.py")
    ...
    try:
        ...
        subprocess.run(["python3", prune_script, "--input", args.input, "--output", pruned_path], env=env, check=True)
        
        print(f"\nStep 2: Voxelizing predictions into OBJ mesh...")
        subprocess.run(["python3", voxelize_script, "--input", pruned_path, "--output", args.output_obj, "--threshold", str(args.threshold)], env=env, check=True)
