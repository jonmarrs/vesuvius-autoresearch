...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the original CT volume (Zarr format)",
    )
    parser.add_argument(
        "--predictions",
        type=str,
        required=True,
        help="Path to the model's predictions (Zarr format)",
    )
    args = parser.parse_args()

    villa_proofreader_path = os.path.abspath(
        "villa/segmentation/vc_proofreader/main.py"
    )
    ...
    cmd = [sys.executable, villa_proofreader_path, "--config", config_path]
    ...
    print(
        "3. Approved patches are automatically saved to local_data/Proofread_Patches/"
    )
