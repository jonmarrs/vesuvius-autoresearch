...
def debug_prediction():
    ...
    ds = VesuviusLabeledDataset(
        ...
        mask,
        patch_size=exp_config.patch_size,
        num_layers=exp_config.num_layers + 8,
        require_ink=True,
    )
    ...
    print(
        f"Prediction stats: max={prob.max():.6f}, min={prob.min():.6f}, mean={prob.mean():.6f}"
    )
    print(
        f"Target stats: max={target.max():.4f}, mean={target.mean():.4f}, sum={target.sum():.4f}"
    )

    dice = (
        (2.0 * torch.sum(target * prob)) / (torch.sum(target) + torch.sum(prob) + 1e-12)
    ).item()
    print(f"Soft Dice: {dice:.6f}")

    thresholded = (prob > 0.5).float()
    hard_dice = (
        (2.0 * torch.sum(target * thresholded))
        / (torch.sum(target) + torch.sum(thresholded) + 1e-12)
    ).item()
