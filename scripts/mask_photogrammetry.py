...
def main():
    parser = argparse.ArgumentParser(description="Segment Scroll and Ruler in Photogrammetry Images")
    parser.add_argument("--input", type=str, required=True, help="Root directory containing the scroll photos (must follow SAM2 folder structure)")
    ...
    try:
        ...
        print("You can now run your preferred Photogrammetry software (e.g. OpenMVG + OpenMVS) on these masked images to build a pristine 3D mesh.")
        print("Once the mesh is built, use scripts/extract_surface_from_mesh.py to generate the final Zarr volume.")
        
    except subprocess.CalledProcessError as e:
        print(f"\nPipeline interrupted or failed: {e}")
        print("Please ensure you have downloaded the fine-tuned SAM2 checkpoint (photo2_ruler_t_1000.torch) into villa/sam2-photogrammetry/checkpoints/")
