...
class VesuviusReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'Vesuvius Autoresearch: Daily Experimental Report', 0, 1, 'C')
...
def generate_pdf():
    ...
    pdf.cell(0, 10, '1. Executive Summary & Insights', 0, 1, 'L')
    ...
    pdf.set_font('helvetica', 'B', 14)
    pdf.cell(0, 10, '2. Performance Metrics & Frontier', 0, 1, 'L')
    ...
    pdf.add_page()
    pdf.set_font('helvetica', 'B', 14)
    pdf.cell(0, 10, '3. Hardware Efficiency Pareto', 0, 1, 'L')
    ...
    if os.path.exists('results.tsv'):
        ...
        if not df.empty:
            ...
            pdf.cell(30, 10, 'Val Dice Loss', 1)
            pdf.cell(30, 10, 'Throughput', 1)
            pdf.cell(30, 10, 'Params (M)', 1)
            pdf.ln()
            
            # Table Rows
            pdf.set_font('helvetica', '', 9)
            for _, row in top_df.iterrows():
                pdf.cell(40, 10, str(row['timestamp'])[:16], 1)
                pdf.cell(30, 10, f"{row['val_bpb']:.6f}", 1)
                pdf.cell(30, 10, f"{row['throughput_Mvps']:.2f}", 1)
                pdf.cell(30, 10, f"{row['num_params_M']:.2f}", 1)
                pdf.ln()
