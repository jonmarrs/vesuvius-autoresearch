...
def generate_pseudo_labels():
    ...
    parser.add_argument("--uris", nargs="+", required=True, help="URIs of unlabeled volumes")
    ...
    for uri in args.uris:
        ...
        for y in tqdm(range(0, H - patch_size + 1, stride), desc="Y-tiles"):
            for x in range(0, W - patch_size + 1, stride):
                block = dataset[z0 : z0 + num_layers, y : y + patch_size, x : x + patch_size]
                x_in = dataset.normalize(block).unsqueeze(0).to(device)
                if x_in.dim() == 4: x_in = x_in.unsqueeze(1) # [B, 1, Z, H, W]
                ...
                full_prob[y : y + patch_size, x : x + patch_size] += prob * weight_window
