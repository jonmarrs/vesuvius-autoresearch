...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Whole-Volume Deformation Metric")
    parser.add_argument("--st-zarr", type=str, required=True, help="Path to the Structure Tensor Zarr volume")
    parser.add_argument("--subsample", type=int, default=1, help="Subsampling factor for faster evaluation")
    ...
    print(f"Metric: Fiber Coherence (based on Structure Tensor Fractional Anisotropy)")
    print(f"Mean FA: {mean_fa:.4f} ± {std_fa:.4f}")
    print(f"Coherence Score: {coherence_score:.2f} / 100")
    print(f"\n[INTERPRETATION]")
    print(f"Run this metric on a volume *before* registration, and then again *after* registration.")
    print(f"An increase in the Coherence Score indicates that the registration successfully")
    print(f"flattened the papyrus structure, aligning the vertical and horizontal fibers.")
