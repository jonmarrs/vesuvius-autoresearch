...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the raw scroll mesh (.obj or .ply)",
    )
    parser.add_argument(
        "--volume",
        type=str,
        required=True,
        help="Path to the original unflattened scroll volume (Zarr or Grid)",
    )
    parser.add_argument(
        "--output",
        type=str,
        required=True,
        help="Output directory for the flattened TIF stack",
    )
    parser.add_argument(
        "--layers",
        type=int,
        default=65,
        help="Total number of layers to extract (depth)",
    )
    ...
    print("--- Vesuvius Autoresearch Surface Extraction ---")
    ...
    cmd = [
        "python3",
        os.path.join(villa_path, "mesh_to_surface.py"),
        "--obj",
        args.mesh,
        "--grid",
        args.volume,
