...
| vesuvius-c | `partial` | `medium` | Low-level CT access for high-throughput chunk reads. | `vesuvius_c_wrapper` ok, `benchmark_vesuvius_c.py` missing, `test_vesuvius_c.py` missing, `SPRINT_KANBAN.md` missing | Run the Vesuvius-C benchmark on local CT chunks before claiming a Progress Prize speedup. |
