...
| --- | --- | --- | --- | --- | --- |
| vesuvius | `partial` | `high` | Official Python CT/Zarr data access and normalization. | `vesuvius_loader.py` missing, `v3_training/trainer.py` ok, `check_loader.py` missing | Keep loader smoke tests aligned with the pinned Villa data API. |
| ink-detection | `partial` | `high` | Official Grand Prize ink model recipes and optimized inference contracts. | `train.py` missing, `predict.py` missing, `scripts/smoke_test_villa_optimized_inference.py` ok | Keep optimized-inference smoke checks in the post-sprint gate before packaging evidence. |
...
| foundation | `unwired` | `medium` | Dataset management and fiber-label assets. | `scripts/labeling/generate_fiber_labels.py` missing | Use fiber assets to expand supervision for hard geometry candidates. |
