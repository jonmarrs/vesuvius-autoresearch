...
def _warp_2d_tensor(t: torch.Tensor, grid2d: torch.Tensor) -> torch.Tensor:
    ...
    if t.dim() == 5:
        ...
        warped = F.grid_sample(
            squeezed, grid2d, mode="bilinear",
            padding_mode="border", align_corners=True,
        )
        return warped.unsqueeze(2)
    return F.grid_sample(
        t, grid2d, mode="bilinear",
        padding_mode="border", align_corners=True,
    )
...
def scroll_decohesion(
    ...
    blurred = F.avg_pool3d(
        x, kernel_size=(blur_kernel, 1, 1), stride=1,
...
def scroll_warping(
    ...
    raw_dy = torch.randn((B, 1, n_ctrl, n_ctrl), device=device, dtype=dtype) * max_displacement
    raw_dx = torch.randn((B, 1, n_ctrl, n_ctrl), device=device, dtype=dtype) * max_displacement

    dy = F.interpolate(raw_dy, size=(H, W), mode="bilinear", align_corners=True).squeeze(1)
    dx = F.interpolate(raw_dx, size=(H, W), mode="bilinear", align_corners=True).squeeze(1)
    ...
    grid2d = torch.stack(
        [grid_x.clamp(-1.0, 1.0), grid_y.clamp(-1.0, 1.0)], dim=-1,
    )  # [B, H, W, 2]
    ...
    x_warped = F.grid_sample(
        x_flat, grid3d, mode="bilinear", padding_mode="border", align_corners=True,
    )
...
def scroll_z_dropout(x: torch.Tensor, drop_rate: float = 0.12) -> torch.Tensor:
    ...
    if x.shape[2] < 3:
        return x
    keep = (torch.rand(
        (x.shape[0], 1, x.shape[2], 1, 1), device=x.device,
    ) > drop_rate).to(dtype=x.dtype)
...
def scroll_intensity_drift(
    ...
    """
    B, C, Z, H, W = x.shape
    depth = torch.linspace(-1.0, 1.0, Z, device=x.device, dtype=x.dtype).view(1, 1, -1, 1, 1)
    slope = torch.empty((B, 1, 1, 1, 1), device=x.device, dtype=x.dtype).uniform_(*slope_range)
    bias = torch.empty((B, 1, 1, 1, 1), device=x.device, dtype=x.dtype).uniform_(*bias_range)
...
def apply_scroll_specific_3d_augmentations(
    ...
    if decohesion_p > 0 and torch.rand((), device=x.device).item() < decohesion_p:
        alpha = torch.empty((), device=x.device, dtype=x.dtype).uniform_(0.15, 0.45).item()
