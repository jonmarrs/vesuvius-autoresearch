...
VILLA_TRAIN_PATH = os.path.abspath(
    os.path.join(PROJECT_ROOT, "villa/segmentation/models/multi-task-3d-unet")
)
...
class VesuviusTrainer(BaseTrainer):
    ...
    def __init__(
        self, config_file: str, verbose: bool = True, debug_dataloader: bool = False
    ):
        ...
        if self.use_uamt:
            ...
            self.labeled_bs = self.mgr.tr_configs.get(
                "labeled_batch_size", self.mgr.train_batch_size // 2
            )

        if self.use_lejepa:
            self.sigreg = SIGRegLoss(
                num_slices=256, lambd=self.mgr.tr_configs.get("sigreg_lambda", 0.02)
            )
    ...
    def _build_dataloaders(self):
        ...
        ds_labeled = VesuviusLabeledDataset(
            ...
            mask_path=os.path.join(PROJECT_ROOT, vol_info.get("mask_volume"))
            if vol_info.get("mask_volume")
            else None,
            patch_size=config.train_patch_size[1],
            num_layers=config.train_patch_size[0],
            is_unlabeled=False,
        ...
        batch_sampler = TwoStreamBatchSampler(
            ...
            config.train_batch_size - self.labeled_bs,
        )
        train_loader = DataLoader(
            ds_combined,
    ...
    def _train_lejepa(self):
        ...
        for epoch in range(self.mgr.max_epoch):
            ...
            for i, (img, _) in enumerate(train_loader):
                ...
                ink_dummy = torch.zeros(
                    (img.shape[0], 1, img.shape[3], img.shape[4]), device=img.device
                )
                fiber_dummy = torch.zeros(
                    (img.shape[0], 1, 1, img.shape[3], img.shape[4]), device=img.device
                )

                img_aug1, _, _ = apply_augmentations(
                    img, ink_dummy, fiber_dummy, global_step, 1000, config=v_cfg_dummy
                )
                img_aug1 = img_aug1.to(self.device)
                _, proj1 = model(img_aug1, return_proj=True)

                # Forward View 2
                img_aug2, _, _ = apply_augmentations(
                    img, ink_dummy, fiber_dummy, global_step, 1000, config=v_cfg_dummy
                )
                ...
                if global_step % 10 == 0:
                    print(
                        f"Step {global_step} | Loss: {loss.item():.4f} | Invar: {loss_dict['invariance_loss']:.4f} | SIGReg: {loss_dict['sigreg_loss']:.4f}"
                    )
        ...
        for epoch in range(self.mgr.max_epoch):
            ...
            for i, (img, target) in enumerate(train_loader):
                ...
                loss_seg = F.binary_cross_entropy_with_logits(
                    outputs[: self.labeled_bs], target[: self.labeled_bs].unsqueeze(1)
                )

                # Consistency with Teacher (EMA)
                with torch.no_grad():
                    # UA-MT: multiple forward passes for uncertainty estimation
                    ema_inputs = img[self.labeled_bs :].repeat(
                        self.uncertainty_T, 1, 1, 1, 1
                    )
                    ema_inputs += torch.clamp(
                        torch.randn_like(ema_inputs) * 0.1, -0.2, 0.2
                    )
                    ...
                    uncertainty = -1.0 * torch.sum(
                        ema_avg_prob * torch.log(ema_avg_prob + 1e-6),
                        dim=1,
                        keepdim=True,
                    )
                ...
                consistency_dist = (
                    torch.sigmoid(outputs[self.labeled_bs :]) - ema_avg_prob
                ) ** 2
                threshold = 0.75 + (1.0 - 0.75) * ramps.sigmoid_rampup(
                    epoch, self.consistency_rampup
                )
                mask = (uncertainty < threshold).float()

                consistency_loss = (consistency_dist * mask).sum() / (mask.sum() + 1e-6)
                consistency_weight = self.consistency_weight * ramps.sigmoid_rampup(
                    epoch, self.consistency_rampup
                )
                ...
                self._update_ema_variables(
                    model, ema_model, self.ema_decay, global_step
                )
                global_step += 1

                if global_step % 10 == 0:
                    print(
                        f"Step {global_step} | Loss Seg: {loss_seg.item():.4f} | Cons: {consistency_loss.item():.4f}"
                    )

    def _update_ema_variables(self, model, ema_model, alpha, global_step):
        alpha = min(1 - 1 / (global_step + 1), alpha)
        for ema_param, param in zip(
            ema_model.parameters(), model.parameters(), strict=False
        ):
